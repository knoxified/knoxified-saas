...
fs.writeFileSync(path.join(__dirname, 'app/automations/page.tsx'), automationsPageStr);
...
Object.entries(systemMappings).forEach(([niche, autoIds]) => {
  ...
  if (fs.existsSync(p)) {
    ...
    // Using simple regex for replacement, since we already have 
    // <SystemAutomations automations={[...]} /> block injected recently.
    
    // Matches the entire block <SystemAutomations automations={[...]} />
    // We'll replace it completely.
    content = content.replace(/<SystemAutomations\s+automations=\{[\s\S]*?\}\s*\/>/g, `<SystemAutomations\s+automations={${payloadStr}}\s+/>`);
    
    // Also fix the system automation component path maybe... wait, let's keep the existing SystemAutomations match.
    // simpler replace:
    // This regex looks for `<SystemAutomations automations={` and closing `} />`
