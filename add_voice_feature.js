...
const voiceHtml = `
            ...
            <div className="p-8 rounded-2xl bg-slate-800/60 backdrop-blur-md border border-slate-700/50 hover:border-cyan-500/30 transition-all shadow-xl">
              ...
              <ul className="space-y-5">
                ...
                <li className="flex items-start gap-3 text-slate-300">
                  <CheckCircle2 className="w-5 h-5 text-cyan-500 mt-1 flex-shrink-0" />
                  <span><strong>Outbound Proactive Calling:</strong> Initiates outbound calls for follow-ups, appointment reminders, and re-engaging lost leads automatically.</span>
                </li>
              </ul>
            </div>`;
...
for (const file of files) {
  ...
  if (gridMatch) {
    // just add it right after the grid opening
