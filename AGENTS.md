...
Pricing UI vs Backend Strategy:

*   Do NOT treat pricing UI labels as database fields.
*   Pricing tiers are merely product packaging.
*   Backend enforcement must use Supabase plan limits.
*   Each plan must map to numeric/boolean backend feature limits (e.g. `voice_minutes`, `automations`, `outbound_capability_flags`).
*   UI features like "Dedicated Success Manager" are display-only and must not be enforced by the backend.
